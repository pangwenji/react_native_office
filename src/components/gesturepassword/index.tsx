...
const getTouchChar = (touch: any,circle:Array<circleProp>) => {
	var x = touch.x;
	var y = touch.y;
	for (let i = 0; i < 9; i++) {
...
const onStart = (event: GestureResponderEvent, gestureState: PanResponderGestureState, circle: Array<circleProp>, setCircle: Function, lines: Array<any>, setLines: Function) => {
	...
	let lastChar = getTouchChar({ x, y },circle);
	if (lastChar) {
		isMoving = true;
		lastIndex = lastChar
		sequence = lastChar;
...
//画直线
const renderLines = (lines:Array<any>,props: any) => {
	let array: Array<any> = [], color;
	let { status, wrongColor, rightColor } = props;
	...
	return (
		<View style={[styles.frame, style, { flex: 1 }]}>
			...
			<View style={styles.board} {..._panResponder.panHandlers}>
				...
				{renderLines(lines, props)}
				<Line ref={lineRef} color={color} start={{
					x: 0,
					y: 0
				}} end={{
					x: 0,
